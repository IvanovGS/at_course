...
class RomanNums:
    ...
    conversion_dict = ((1000, 'M'), (900, 'CM'), (500, 'D'), (400, 'CD'),
                       (100, 'C'), (90, 'XC'), (50, 'L'), (40, 'XL'),
                       (10, 'X'), (9, 'IX'), (5, 'V'), (4, 'IV'), (1, 'I'))

    def from_roman(self):
        arab = 0
        for arabic, roman in conversion_dict:
            while self.roman_number.startswith(roman):
                arab += arabic
                self.roman_number = self.roman_number[len(roman):]
        return arab

    def is_palindrome(self):
        arab = str(self.from_roman())
        if arab == ''.join(reversed(list(arab))):
            return True
        else:
            return False
