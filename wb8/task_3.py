...
def segment(x, y):
    '''
    функция проверяет данные координат на корректность и возвращает сумму всех координат,
    в случае некорректных данных возвращает текст исключения в обратном порядке
    :param x: Кортеж из координат точки х
    :param y: Кортеж из координат точки у
    :return:
